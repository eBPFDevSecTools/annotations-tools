...
'''
NOTE: We remove functions which do not have an entry in 
capdict(which is generated by TXL static analyzer) 
from the list of called functions.
'''
def add_root_and_level_info(capdict):
    q = []
    level_dict = dict()
    map1 = dict()
    map2 = dict()
...
'''
1. IP: Takes a repo path and commit_id and checksout the code and puts it in src_dir
2. OP: Generates comment stub for the code in src_dir and puts it in op_dir, also takes txl_op_dir

JSON SCHEMA for the comment stub here ...

'''
def gen_comment_stubs_w_src_code():
    return

'''
1. IP: Human annotated source_code folder, txl_op_dir from previous step ??
2. OP: JSON format annotations file for each function in the source code folder
3. JSON SCHEMA for the annotations_json
'''
def gen_human_annotations_from_annotate_src_code():
    # call json validate to ensure that the json is valid.
    return



'''
1. IP: Takes a repo path and commit_id and checksout the code and puts it in src_dir (optionally the location of src code checked out)
2. OP: JSON format OPENED annotations file for each function in the source code folder
3. JSON SCHEMA for the OPENED_annotations_json
'''
def gen_opened_annotations_from_src_code():
    return

'''
1. IP: Takes a repo path and commit_id and checksout the code and puts it in src_dir (optionally the location of src code checked out)
2. OP: JSON format AI annotations file for each function in the source code folder
3. JSON SCHEMA for the AI_annotations_json
'''
def gen_llm_annotations_from_src_code():
    return
