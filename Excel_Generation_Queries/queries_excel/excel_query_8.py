...
def get_number_of_functions(index_name, client):
    ...
    for repo in repos:
        for func in root_funcs:
            ...
            map_counts.loc[func][repo] = list(READONLY_MAPS)
            map_counts.loc[func][f"{repo}_count"] = len(READONLY_MAPS)

        maps_ = set()
        for maps_list in map_counts[repo]:
            for f in maps_list:
                maps_.add(f)
    ...
    map_counts.to_csv("../Query_CSVs/excel_query_8_results.csv")
...
if __name__ == "__main__":
    ...
    print(f"Generated Excel for Query 8 - {num_repos} repos")
    print(num_maps)